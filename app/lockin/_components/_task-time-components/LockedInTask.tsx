...
const LockedInTask = () => {
  ...
  const { handleStartTask, handlePauseTask, handleCompleteTask } =
    useTaskStore();
  ...
  return (
    <div className="w-full min-h-24 flex items-center align-middle justify-center space-x-5 rounded-xl outline-2 outline-app-bg">
      ...
      <h1 className="font-semibold md:text-2xl text-xl text-center">
        {" "}
        {startedFocusedTask ? "🔒" : "🔓"} {focusedTask?.name}
      </h1>
      {!startedFocusedTask ? (
        <CiPlay1
          className={`text-4xl btn-hover flex-none ${loading && "hover:cursor-progress"}`}
          onClick={async () => {
            if (loading) return;
            setLoading(true);
            await handleStartTask(focusedTask!);
